...
Flow:
  ...
  5. Create alerts (with 10-min cooldown dedup)
...
async def _run_drift_check(model_id: int) -> dict:
    ...
    async with AsyncSessionLocal() as db:
        ...
        for feat_name, current_values in feature_data.items():
            ...
            # Reconstruct baseline array from stored percentiles / histogram
            # (real baseline values aren't stored — we sample from the stored histogram)
            ...
            # Synthetic baseline: bin midpoints repeated proportionally
        ...
        # CUSUM on prediction scores
        try:
            baseline_pred = np.random.normal(0.5, 0.1, 500)  # placeholder baseline mean
            det = CUSUMDetector()
            det.fit(baseline_pred, feature_name="prediction")
        ...
        # Page-Hinkley on prediction scores
        try:
        ...
        try:
            if feature_names:
                matrix = np.column_stack([
                    np.array(feature_data[f]) for f in feature_names
                    if len(feature_data[f]) == len(feature_data[feature_names[0]])
                ])
                if matrix.shape[0] >= 50:
                    baseline_matrix = np.random.randn(500, matrix.shape[1])
                    det = IForestDetector(contamination=0.05)
                    det.fit(baseline_matrix)
                    score = det.score(matrix)
                    drift_scores.setdefault("isolation_forest", []).append(("multivariate", score))
                    if score < -0.2:
